##################################################################
######## SUBJECT SPECIFIC PARAMETERs ESTIMATION FOR SECTION 4.1.2
##################################################################
...
load(paste('true_trajectory_linmodel_2dim_misspe_pop_',nb_subject,'.Rdata',sep=""))
...
for (nb_t  in 1:nb_trial){
  ...
  for (nbs in 1:nb_subject){
    ...
    for (nb_l  in 1:nb_tested_U){
      ...
      matplot(Time_real_true_trajectory_i,True_trajectory_i[1,],type="l" ,xlab = "Time", ylab = "X",main = main_title,col="red",ylim= c(0,1.2*max(True_trajectory_i)))
