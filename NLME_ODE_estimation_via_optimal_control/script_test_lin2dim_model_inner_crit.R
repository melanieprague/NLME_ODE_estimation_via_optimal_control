##################################################################
######## SUBJECT SPECIFIC PARAMETERs ESTIMATION FOR SECTION 4.1.1
##################################################################
...
# test data retrieval
nb_subject =20
load(paste('data_gen_linmodel_2dim_pop',nb_subject,'.Rdata',sep=""))
...
lambda_seq = c(10,10^2,10^3,10^4,10^5)
  ...
  for (nbs in 1:nb_subject){
    ...
    for (nb_l  in 1:nb_tested_U){
      ...
      list_inner_arg = list(Times_integ_i, pseudo_Y_i, weight_integ_i,true_bi,true_Delta_mat,mat_U,mat_A_pop,vect_r_pop,mat_B,mat_C, known_x0i,2,0
                            ,true_theta,list(),type_algorithm_optim = 1,c(200,1))
      ...
      out <- ode( y =  true_x0i,times = Times_integ_i,func = linmodel_2dim_sim, parms =c(true_theta, true_bi))
      matplot(Times_integ_i,out[,2],type="l" ,xlab = "Time", ylab = "X",main = main_title,col="red",ylim= c(0,1.2*max(out[,2])))
