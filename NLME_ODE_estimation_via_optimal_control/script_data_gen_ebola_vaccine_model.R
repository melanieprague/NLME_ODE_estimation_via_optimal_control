...
# specification of scaling parameter for Ab value, population size and observation numbers
nb_subject =20
coeff_mult = 0.01
...
# data file name
rep_file = paste('data_gen_ebola_vaccine_model_pop',nb_subject,'.Rdata',sep="")
...
for (nbt in 1:nb_trial){
  ...
  for (nbs in 1:nb_subject){
    ...
    out <- ode( y = x0i, times = Times_obs,func =Ebola_vaccine_model_sim , parms =c(theta_gen,true_bi))
    out_ref <- ode(func = Ebola_vaccine_model_sim , y = x0i, parms =c(theta_gen,true_bi), times = Times_ref)
    
    Y_noise =matrix(coeff_mult*out[,2] +std*rnorm(n = nb_obs,0),1,nb_obs)
    
    Obs_subjects = append(Obs_subjects,list(list(Times_obs,Y_noise,list(log_delta_S,log_delta_L,coeff_mult))))
    ...
    if (nbt ==1){
      if (nbs==1){
        matplot(Times_ref ,coeff_mult*out_ref[,2], type = "l", xlab = "Time", ylab = "",main = "Ebola vaccine ODE model",ylim = c(0,coeff_mult*2*10^4))
        points(Times_obs,Y_noise[1,],pch=16,bg="black") 
      }else{
        lines(Times_ref,coeff_mult*out_ref[,2],lty=1) 
        points(Times_obs,Y_noise[1,],pch=16,bg="black") 
      }
      legend("topright",c("Ab","Observations"),pch=c(NA,16),lty=c(1,NA),bg=c(NA,"black"),col=c("black", "black"), lwd = c(1,1))
