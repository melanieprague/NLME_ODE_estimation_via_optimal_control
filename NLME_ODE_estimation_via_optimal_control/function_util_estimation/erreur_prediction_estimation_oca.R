erreur_prediction_estimation_oca_lincase <- function(Times_integ_set,pseudo_Y_set,weight_integ_set,param_sub_ini_set,param_exogen_set, Delta_mat,param_pop_est, mat_U,mat_A_pop,vect_r_pop,mat_B,mat_C,list_x0_known = list()){
  ############################################################
  # DESCRIPTION: compute the summed prediction error corresponding to the current population and subject specific parameter estimations for linear ODE based mixed-effect models
  # 
  # FUNCTION SIGNATURE:  erreur_prediction_estimation_oca_lincase <- function(Times_integ_set,pseudo_Y_set,weight_integ_set,param_sub_ini_set,param_exogen_set, Delta_mat,param_pop_est, mat_U,mat_A_pop,vect_r_pop,mat_B,mat_C,list_x0_known = list())
  # 
  # INPUT :
  ...
  v_field <- function(Time, State, Pars,nbs_cur){
    dState = mat_A_pop(Time,param_sub_ini_set[[nbs_cur]],param_pop_est,param_exogen_set[[nbs_cur]])%*%State+ vect_r_pop(Time,param_sub_ini_set[[nbs_cur]],param_pop_est,param_exogen_set[[nbs_cur]])
    ...
    for (nbs  in 1:nb_subject){
      ...
      seq_argument_nbs = list(Times_integ_set_nbs,pseudo_Y_set_nbs,weight_integ_set_nbs,param_sub_ini_set_nbs, Delta_mat,
      ...
      out_inner_control_problem =est_param_oca_inner_criteria_lincase_par_version(seq_argument_nbs)
