est_param_oca_outer_criteria_prof_par_version <- function(Subject_data,State_ini_set,param_pop_ini,log_Delta_vect_ini,mat_U,mat_A,vect_r_pop,mat_B,mat_C,
                                                           ...
                                                           ,type_optim=0,est_var=0,type_optim_inner=0,nb_iter_max = c(1000,200,0),cl=list()){
  
  ############################################################
  # DESCRIPTION: estimation of the mean population parameter and log-transformed of the subject specific variance via outer criteria optimization for ODE based mixed-effect models
  # 
  # FUNCTION SIGNATURE:  est_param_oca_outer_criteria_prof_par_version <- function(Subject_data,State_ini_set,param_pop_ini,log_Delta_vect_ini,mat_U,mat_A,vect_r_pop,mat_B,mat_C,
  #                                              mesh_iter,set_theta_sub,list_x0_known = list(),est_pop_parameter_only=1,delta_known=0,log_prior=function(param_pop,Delta_mat){return(0)}
  #                                                 ,type_optim=0,est_var=0,nb_iter_max = c(1000,200,0),cl=list())
  # 
  # INPUT :
  ...
  # State_ini_set             :(list of dim_syst x nb_obs sized matrix) list of initial state value at observation points for the optimal control  for each subject
  ...
  # mesh_iter                 :(integer) refinment of the mesh size, nulmber of added dicretization points between two observation points  
  ...
  # est_var                   :(integer) spefify if the variance is estimated, 1 if so, 0 otherwise
  ...
  for (nbs in 1:nb_subject){
    ...
    State_ini_set_s =State_ini_set[[nbs]]
    ...
    pseudo_State_ini_s_cur =matrix(data = rep(0,dim_syst*nb_time_integ_s ), nrow = dim_syst, ncol = nb_time_integ_s )
    ...
    for (obs_i in 1:(length(Times_obs_s)-1)){
      ...
      pseudo_State_ini_s_cur[,seq(mesh_iter*(obs_i-1)+1,(mesh_iter*obs_i+1),by =1)]= matrix(rep(State_ini_set_s[,obs_i],mesh_iter+1),dim_syst,mesh_iter+1)
    ...
    pseudo_State_ini_set = append(pseudo_State_ini_set,list(pseudo_State_ini_s_cur))
    param_exogen_set = append(param_exogen_set,list(Subject_data[[nbs]][[3]]))
  }
  
  nb_obs_tot = dim_obs*nb_obs_tot;
  param_sub_ini = set_theta_sub
  pseudo_State_ini =  pseudo_State_ini_set
  
  
  
  list_arguments_refined = list()
  for (nbs1  in 1:nb_subject){
    seq_argument_nbs1_refined = list(Times_integ_set[[nbs1]],pseudo_Y_set[[nbs1]],pseudo_State_ini[[nbs1]],weight_integ_set[[nbs1]],param_sub_ini[[nbs1]], diag(exp(log_Delta_vect_ini),length(log_Delta_vect_ini),length(log_Delta_vect_ini)),
                             mat_U,mat_A,vect_r_pop,mat_B,mat_C,list_x0_known[[nbs1]],2,est_pop_parameter_only,param_pop_ini,param_exogen_set[[nbs1]],type_optim_inner,nb_iter_max[2:3])
    list_arguments_refined = append(list_arguments_refined,list(seq_argument_nbs1_refined))
  }
  
  
  if (length(cl)==0){
    out_parLapply_refined =  lapply(list_arguments_refined,est_param_oca_inner_criteria_par_version)
  }else{
    out_parLapply_refined =  parLapply(cl,list_arguments_refined,est_param_oca_inner_criteria_par_version) 
  }
  
  param_sub_set_refined = list()
  pseudo_State_ini_refined = list()
  for (nbs2  in 1:nb_subject){
    #   print( out_parLapply[[nbs2]]$val_h_s)
    theta_sub_s = out_parLapply_refined[[nbs2]]$bi_est
    State_opt_s = out_parLapply_refined[[nbs2]]$Xi_opt
    param_sub_set_refined  = append( param_sub_set_refined ,list(theta_sub_s))
    pseudo_State_ini_refined = append( pseudo_State_ini_refined,list(State_opt_s))
  }
  
  func_eco_E <- function(param_pop,Delta_mat,pseudo_State,param_sub_0){
    
    val_h =  matrix(data = 0, nrow = nb_subject, ncol =1)
    list_ui_opt = list()
    list_Xi_opt = list()
    ...
    list_arguments = list()
    for (nbs1  in 1:nb_subject){
      seq_argument_nbs1 = list(Times_integ_set[[nbs1]],pseudo_Y_set[[nbs1]],pseudo_State[[nbs1]],weight_integ_set[[nbs1]],param_sub_0[[nbs1]], Delta_mat,
                               mat_U,mat_A,vect_r_pop,mat_B,mat_C,list_x0_known[[nbs1]],2,est_pop_parameter_only,param_pop,param_exogen_set[[nbs1]],type_optim_inner,nb_iter_max[2:3])
      list_arguments = append(list_arguments,list(seq_argument_nbs1))
    }
    
    param_sub_set = list()
    State_opt_sub_set = list()
    
    if (length(cl)==0){
      out_parLapply =  lapply(list_arguments,est_param_oca_inner_criteria_par_version)
    }else{
      out_parLapply =  parLapply(cl,list_arguments,est_param_oca_inner_criteria_par_version) 
    }
  ...
  func_eco <- function(param_c){
    ...
    out_eco_E = func_eco_E(param_pop_cur,Delta_mat_cur, pseudo_State_ini_refined ,  param_sub_set_refined )
  ...
  if (type_optim==0){
    ...
    res_estim = optimr(param_ext_ini, func_eco, gr=NULL, hess=NULL, lower=-Inf, upper=Inf,control = list(trace=1),method='nlminb') 
  }
  ...
  out_var  = list()
  erreur_prediction_estimation_mat_U_cur = 10^20
  out_try_catch <- tryCatch({
    
    erreur_prediction_estimation_mat_U_cur = erreur_prediction_estimation_oca(Times_integ_set,pseudo_Y_set,list_Xi_opt,weight_integ_set,list_bi_est,param_exogen_set, 
                                                                                     Delta_mat_est, theta_est, mat_U,mat_A,vect_r_pop,mat_B,mat_C,list_x0_known)
  ...
  #  out_try_catch <- tryCatch({
    ...
    out_var =est_param_oca_variance_estimation_prof_par_version(Times_integ_set,pseudo_Y_set,list_Xi_opt ,param_exogen_set,weight_integ_set,theta_est,delta_est,list_bi_est,
                                                                     mat_U,mat_A,vect_r_pop,mat_B,mat_C,nb_obs_tot,list_x0_known,est_pop_parameter_only,delta_known,method_est_jacob =1,cl)
       # },error = function(cond){
      #   out_var  = list()
      #   return(1)}
      # )
