##################################################################
######## SUBJECT SPECIFIC PARAMETERS ESTIMATION FOR SECTION 4.2.1
##################################################################
...
lambda_seq = c(10,1000,10^6,10^9)
...
for (nb_t  in 1:nb_trial){
  ...
  for (nbs in 1:nb_subject){
    ...
    for (nb_l  in 1:nb_lambda){
      ...
      out <- ode( y =  true_x0i,times =  Times_integ_i,func =insulin_model_sim, parms =c(theta_gen,true_bi,coeff_mult_gi,coeff_mult_x))
      matplot( Times_integ_i,out[,2:3],type="l" ,xlab = "Time", ylab = "X",main = main_title,col="red",ylim= c(0,1.2*max(out[,2:3])))
